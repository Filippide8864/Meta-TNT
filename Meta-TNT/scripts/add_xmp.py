...
def add_xmp(file_path, metadata):
    try:
        print(f"Aggiunta dei metadati XMP a: {file_path}")
        subprocess.run(['exiftool', '-overwrite_original'] + metadata + [file_path], check=True)
        print(f"Metadati XMP aggiunti a: {file_path}")
    except subprocess.CalledProcessError as e:
        print(f"Errore durante l'aggiunta dei metadati XMP a {file_path}: {e}")
...
def process_directory(directory, metadata):
    ...
    with ThreadPoolExecutor() as executor:
        executor.map(lambda file: add_xmp(file, metadata), files)
