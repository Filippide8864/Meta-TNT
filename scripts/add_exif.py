...
def add_exif(file_path, metadata):
    try:
        print(f"Aggiunta dei metadati EXIF a: {file_path}")
        subprocess.run(['exiftool', '-overwrite_original'] + metadata + [file_path], check=True)
        print(f"Metadati EXIF aggiunti a: {file_path}")
    except subprocess.CalledProcessError as e:
        print(f"Errore durante l'aggiunta dei metadati EXIF a {file_path}: {e}")
